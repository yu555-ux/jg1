...
export const Schema = z.object({
  ...
  角色基础: z
    ...
    .transform(data => {
      ...
      const idx = Math.min(9, Math.floor(corruption / 100));

      // 依存评价逻辑
      const dependency = data.依存度 || 0;
      const depStages = ['反抗', '敌视', '戒备', '中立', '顺从', '依赖', '痴迷', '狂热'];
      const depIdx = Math.min(7, Math.floor((dependency + 200) / 50));
  ...
  当前功法: z
    ...
    .transform(data => {
      const processSkill = item => {
        ...
        const idx = Math.min(4, Math.floor(prof / 200));
        return { ...item, 掌握程度: stages[idx] };
      };

      return {
        主修: processSkill(data.主修),
        // 保险截断：不管传入多少个，强行只保留最多 3 个辅修
    ...
    .object({
      上衣: z
        .object({
          ...
          分类: z.string().prefault('着装'),
          着装类型: z.string().prefault('上衣'),
          品阶: z.string().prefault('凡阶'),
          描述: z.string().prefault('暂无描述'),
          固定加成: z.array(z.string()).prefault([]),
          效果: z.array(z.string()).prefault([]),
  ...
  储物空间: z
    .record(
      ...
      z.object({
        ...
        灵石类型: z.string().prefault(''),
        着装类型: z.string().prefault(''),
        招式: z
          .array(
            z.object({
